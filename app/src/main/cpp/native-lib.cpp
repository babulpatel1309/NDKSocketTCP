...
JNICALL
Java_com_testproject_server_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
JNICALL
Java_com_testproject_server_MainActivity_initiateTcpConnection(JNIEnv *env, jobject javaThis) {
    int tcp_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (tcp_socket < 0) {
        return (env)->NewStringUTF("ERROR CREATING SOCKET");
    }
    const char *server_host = "192.168.232.2";
